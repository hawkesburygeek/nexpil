...
import { TASKTYPE } from '../../assignedTask/enum_task';
...
function DoctorAssignedTask(props) {
    ...
    const { isNew, selectedGroup, taskTemplateList, onSubmitTaskGroup, setSelectedGroup, setIsLoading } = props;
    ...
    const onDeleteTask = async (index, taskInfo) => {
        if (await showConfirm({
            ...
            if (taskInfo.id === undefined) {
                _tempTaskList = tempTaskList.filter((x, index) => x !== index);
